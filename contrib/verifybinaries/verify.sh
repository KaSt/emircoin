#!/usr/bin/env bash
# Copyright (c) 2016 The Emircoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   emircoincore.org and emircoin.org and compares them.
...
WORKINGDIR="/tmp/emircoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://emircoincore.org"
HOST2="https://emircoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="emircoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'emircoin-',
...
if [ $? -ne 0 ]; then
   echo "emircoin.org failed to provide signature file, but emircoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "emircoin.org and emircoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Emircoin Core binary release signing key installed?"
   fi
