...
const char * const EMIRCOIN_CONF_FILENAME = "emircoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "emircoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Emircoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Emircoin
    // Mac: ~/Library/Application Support/Emircoin
    // Unix: ~/.emircoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Emircoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Emircoin";
#else
    // Unix
    return pathRet / ".emircoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EMIRCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Emircoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Emircoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Emircoin Core developers";
    }
