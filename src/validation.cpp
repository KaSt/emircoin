...
#if defined(NDEBUG)
# error "Emircoin cannot be compiled without assertions."
#endif
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s EMC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    LogPrintf("GetBlockSubsidy: Subsidy before touching anything %i\n", nSubsidy);
    if (nHeight < 3 ) {
    	return nSubsidy;
    }
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 4 years.
    // Additionally, to possibly discourage pure speculation, the Subsidy is calculated based on the 
    // performance (speed in solving) the previous block. We want to keep a 20 minutes window between blocks
    // we don't care about strong hashing power, any device should be able to mine and participate to
    // the network. In case somehody is faster than others, Subsdidy is lowered.
    CBlockIndex* pindexPrev = ::ChainActive().Tip();
    LogPrintf("GetBlockSubsidy: value of pindexPrev is null?  %s\n", (pindexPrev == nullptr ? "True" : "False"));
    nSubsidy >>= halvings;
    uint32_t timeDiff = pindexPrev->nTime - pindexPrev->pprev->nTime;
    LogPrintf("GetBlockSubsidy: time diff between blocks is %i\n", timeDiff); 
    float nRatio;
    if (timeDiff > (20 * 60)) {
    	LogPrintf("GetBlockSubsidy: miner spent more than expected, granting full subsidy\n");
	nRatio = 1;
    } else {
      LogPrintf("GetBlockSubsidy: miner was faster than expected, granting ratio of subsidy based on time spent mining\n");	
      nRatio = ((float)timeDiff / (20 * 60));
    }
    LogPrintf("GetBlockSubsidy: ratio before adjustement: %f\n", nRatio);
    if (nRatio > 1) {
    	nRatio = 1;
    }
    LogPrintf("GetBlockSubsidy: ratio after adjustement: %f\n", nRatio);
    nSubsidy = (int)(nSubsidy * nRatio);
    LogPrintf("GetBlockSubsidy: Final Subsidy is %i\n",nSubsidy); 
    return nSubsidy;
}
...
bool IsInitialBlockDownload()
{
	...
	LogPrintf("Tip is null, let's download blocksn\n");
        return true;
    }
    if (::ChainActive().Tip()->nChainWork < nMinimumChainWork) {
	LogPrintf("Minimum chainwork is not enought, let's download blocks\n");
        return true;
    }
    if (::ChainActive().Tip()->GetBlockTime() < (GetTime() - nMaxTipAge * 40000)) {
    	LogPrintf("Something off witrh the time, let's download blocks\n");
        return true;
    }
...
{
    LOCK(cs_main);
    LogPrintf("IsWitnessEnabled\n");
    if (pindexPrev != nullptr) {
        LogPrintf("IsWitnessEnabled %i \n", pindexPrev->nHeight);  
    	if ( pindexPrev->nHeight > 2000) {
    		return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == ThresholdState::ACTIVE);
    	} 
    }
    LogPrintf("Returning from IsWitnessEnabled\n");
    return false;
}
...
{
    LOCK(cs_main);
    LogPrintf("IsNullDummyEnabled\n");
    if (pindexPrev != nullptr && pindexPrev->nHeight  > 2000) {
    	return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_SEGWIT, versionbitscache) == ThresholdState::ACTIVE);
    } else {
    	return true;
    }
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
	...
	LogPrintf("ContextualCheckBlock\n");
        if ( nHeight > 2000 && (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
    ...
    LogPrintf("fHaveWitness %s \n", fHaveWitness ? "true" : "false");
    // No witness data is allowed in blocks hat don't commit to witness data, as this would otherwise leave room for spam
    if (!fHaveWitness) {
      for (const auto& tx : block.vtx) {
            if (tx->HasWitness()) {
